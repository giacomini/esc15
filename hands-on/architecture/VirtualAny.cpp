...
#elif VARIANT
  using AnyBC = AnyOfP<Base,B,C>;
  std::vector<AnyBC> anys; anys.reserve(2*size);
  for (int i=0;i<size;++i) { anys.emplace_back(BuildAnyOf<AnyBC,C>::build(3.14));anys.emplace_back(BuildAnyOf<AnyBC,B>::build(7.1));}
  std::random_shuffle(anys.begin(),anys.end());  

  std::cout << "using variant" << std::endl;
  for (int i=0; i<20000; ++i) {
    for (auto const & p : anys) c += p().type==3 ? static_cast<C const&>(p()).comp() : static_cast<B const&>(p()).comp();
  }
#else
